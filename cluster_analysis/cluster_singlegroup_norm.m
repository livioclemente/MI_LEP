...
for t1 = 1:Nt
    for t2 = (t1+1):Nt
        ...
        for i = 1:nperm
            ...
            hp_norm(t1, t2, i) = II_perm;
        end
    end
end

% Store results
h0_values_norm = h0_norm;
hp_values_norm = hp_norm;
...
h0_norm = h0_norm + h0_norm';
for iperm = 1:nperm 
    hp_norm(:,:,iperm) = hp_norm(:,:,iperm) + hp_norm(:,:,iperm)';
end 
...
for iperm = 1:nperm
    perms = true(1,nperm);
    perms(iperm) = 0;
    zvals = squeeze((hp_norm(:,:,iperm) - mean(hp_norm(:,:,perms),3)) ./ std(hp_norm(:,:,perms),[],3));
...
zmap_pth = squeeze((h0_norm - mean(hp_norm, 3)) ./ std(hp_norm, [], 3));
...
imagesc(time_plot, time_plot, h0_norm);
hold on;
...
lim = max(abs(min(min(h0_norm))), max(max(h0_norm)));
caxis([-lim, lim]);
