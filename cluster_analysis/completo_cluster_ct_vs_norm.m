...
% Save file
save('ct_vs_norm_results.mat', 'diff_h0', 'diff_perm_t_values', 'clust_max', 'zmap');
...
[~, ~, ~, stats] = ttest2(erp_ct, erp_norm);
t_values_erp_ctvsnorm = stats.tstat;

% Permuta i dati e calcola i valori t per ogni permutazione
perm_t_values_erp_ctvsnorm = zeros(num_perms, Nt);
for p = 1:num_perms
    perm_data = [erp_ct; erp_norm];
    perm_data = perm_data(randperm(size(perm_data, 1)), :);
    perm_data_ct = perm_data(1:size(erp_ct, 1), :);
    perm_data_norm = perm_data(size(erp_ct, 1)+1:end, :);
    
    for sample = 1:Nt
        [~, ~, ~, stats] = ttest2(perm_data_ct(:, sample), perm_data_norm(:, sample));
        perm_t_values_erp_ctvsnorm(p, sample) = stats.tstat;
    end
end

% Calcola i p-value
p_values_erp_ctvsnorm = sum(perm_t_values_erp_ctvsnorm > repmat(t_values_erp_ctvsnorm, num_perms, 1)) / num_perms;

% Confronta i p-value con il livello alpha
significantly_different_erp_ctvsnorm = p_values_erp_ctvsnorm < alpha;

% Stampa i risultati
for sample = 1:Nt
    if significantly_different_erp_ctvsnorm(sample)
        fprintf('Il campione %d è significativamente diverso tra le due condizioni (p = %.4f).\n', sample, p_values_erp_ctvsnorm(sample));
    else
...
threshold_erp_ctvsnorm = quantile(perm_t_values_erp_ctvsnorm(:), 0.975);

% Identifica i cluster di campioni adiacenti per i quali il valore t supera la soglia
clusters_pos_erp_ctvsnorm = bwconncomp(t_values_erp_ctvsnorm > threshold_erp_ctvsnorm);
clusters_neg_erp_ctvsnorm = bwconncomp(t_values_erp_ctvsnorm < -threshold_erp_ctvsnorm);

% Calcola la statistica del cluster per ogni cluster come la somma dei valori t nel cluster
masses_pos_erp_ctvsnorm = cellfun(@(I) sum(t_values_erp_ctvsnorm(I)), clusters_pos_erp_ctvsnorm.PixelIdxList);
masses_neg_erp_ctvsnorm = cellfun(@(I) sum(abs(t_values_erp_ctvsnorm(I))), clusters_neg_erp_ctvsnorm.PixelIdxList);

% Trova il cluster positivo/negativo più grande
[max_mass_pos_erp_ctvsnorm, ~] = max(masses_pos_erp_ctvsnorm);
[max_mass_neg_erp_ctvsnorm, ~] = max(masses_neg_erp_ctvsnorm);

% Assicurati che time_axis_corrected_ct abbia la stessa lunghezza di t_values
assert(length(time_axis_corrected) == length(t_values_erp_ctvsnorm), 'Time axis and t values must have the same length');

% Trova l'indice del cluster positivo/negativo più grande
[~, idx_max_mass_pos_erp_ctvsnorm] = max(masses_pos_erp_ctvsnorm);
[~, idx_max_mass_neg_erp_ctvsnorm] = max(masses_neg_erp_ctvsnorm);

% Save variables for plot
save('ERP_cluster_ct_vs_norm.mat', 'time_axis_corrected', 't_values_erp_ctvsnorm', 'threshold_erp_ctvsnorm', 'clusters_pos_erp_ctvsnorm', 'clusters_neg_erp_ctvsnorm', 'idx_max_mass_pos_erp_ctvsnorm', 'idx_max_mass_neg_erp_ctvsnorm');
...
% Calcola i valori t per ogni timepoint tra data_ct e data_pth
[~, ~, ~, stats] = ttest2(MI_ct, MI_norm);
t_values_MI_ctvsnorm = stats.tstat;

% Permuta i dati e calcola i valori t per ogni permutazione
perm_t_values_MI_ctvsnorm = zeros(num_perms, Nt);
for p = 1:num_perms
    perm_data = [MI_ct; MI_norm];
    perm_data = perm_data(randperm(size(perm_data, 1)), :);
    perm_data_ct = perm_data(1:size(MI_ct, 1), :);
    perm_data_norm = perm_data(size(MI_ct, 1)+1:end, :);
    
    for sample = 1:Nt
        [~, ~, ~, stats] = ttest2(perm_data_ct(:, sample), perm_data_norm(:, sample));
        perm_t_values_MI_ctvsnorm(p, sample) = stats.tstat;
    end
end

% Calcola i p-value
p_values_MI_ctvsnorm = sum(perm_t_values_MI_ctvsnorm > repmat(t_values_MI_ctvsnorm, num_perms, 1)) / num_perms;

% Confronta i p-value con il livello alpha
significantly_different_MI_ctvsnorm = p_values_MI_ctvsnorm < alpha;

% Stampa i risultati
for sample = 1:Nt
    if significantly_different_MI_ctvsnorm(sample)
        fprintf('Il campione %d è significativamente diverso tra le due condizioni (p = %.4f).\n', sample, p_values_MI_ctvsnorm(sample));
    else
        fprintf('Il campione %d non è significativamente diverso tra le due condizioni (p = %.4f).\n', sample, p_values_MI_ctvsnorm(sample));
    end
end
...
% Soglia per l'identificazione dei cluster - qui sto usando la soglia che hai calcolato precedentemente
threshold_MI_ctvsnorm = quantile(perm_t_values_MI_ctvsnorm(:), 0.975);

% Identifica i cluster di campioni adiacenti per i quali il valore t supera la soglia
clusters_pos_MI_ctvsnorm = bwconncomp(t_values_MI_ctvsnorm > threshold_MI_ctvsnorm);
clusters_neg_MI_ctvsnorm = bwconncomp(t_values_MI_ctvsnorm < -threshold_MI_ctvsnorm);

% Calcola la statistica del cluster per ogni cluster come la somma dei valori t nel cluster
masses_pos_MI_ctvsnorm = cellfun(@(I) sum(t_values_MI_ctvsnorm(I)), clusters_pos_MI_ctvsnorm.PixelIdxList);
masses_neg_MI_ctvsnorm = cellfun(@(I) sum(abs(t_values_MI_ctvsnorm(I))), clusters_neg_MI_ctvsnorm.PixelIdxList);

% Trova il cluster positivo/negativo più grande
[max_mass_pos_MI_ctvsnorm, ~] = max(masses_pos_MI_ctvsnorm);
[max_mass_neg_MI_ctvsnorm, ~] = max(masses_neg_MI_ctvsnorm);

% Assicurati che time_axis_corrected_ct abbia la stessa lunghezza di t_values
assert(length(time_axis_corrected) == length(t_values_MI_ctvsnorm), 'Time axis and t values must have the same length');

% Trova l'indice del cluster positivo/negativo più grande
[~, idx_max_mass_pos_MI_ctvsnorm] = max(masses_pos_MI_ctvsnorm);
[~, idx_max_mass_neg_MI_ctvsnorm] = max(masses_neg_MI_ctvsnorm);

% Save variables for plot
save('MI_cluster_ct_vs_norm.mat', 'time_axis_corrected', 't_values_MI_ctvsnorm', 'threshold_MI_ctvsnorm', 'clusters_pos_MI_ctvsnorm', 'clusters_neg_MI_ctvsnorm', 'idx_max_mass_pos_MI_ctvsnorm', 'idx_max_mass_neg_MI_ctvsnorm');
...
plot(time_plot, t_values_erp_ctvsnorm, 'k');
hold on;
...
line([min(time_axis_corrected), max(time_axis_corrected)], [threshold_erp_ctvsnorm, threshold_erp_ctvsnorm], 'Color', 'k', 'LineStyle', '--');
line([min(time_axis_corrected), max(time_axis_corrected)], [-threshold_erp_ctvsnorm, -threshold_erp_ctvsnorm], 'Color', 'k', 'LineStyle', '--');

try
    [~, idx_max_mass_pos_erp_ctvsnorm] = max(masses_pos_erp_ctvsnorm);
    for idx = clusters_pos_erp_ctvsnorm.PixelIdxList{idx_max_mass_pos_erp_ctvsnorm}
        area(time_axis_corrected(idx), t_values_erp_ctvsnorm(idx), 'FaceColor', 'r', 'EdgeColor', 'none');
    end
catch
...
try
    [~, idx_max_mass_neg_erp_ctvsnorm] = max(masses_neg_erp_ctvsnorm);
    for idx = clusters_neg_erp_ctvsnorm.PixelIdxList{idx_max_mass_neg_erp_ctvsnorm}
        area(time_axis_corrected(idx), t_values_erp_ctvsnorm(idx), 'FaceColor', 'b', 'EdgeColor', 'none');
    end
catch
...
plot(time_axis_corrected, t_values_MI_ctvsnorm, 'k');
hold on;
xlabel('Tempo');
ylabel('Valore t');

line([min(time_axis_corrected), max(time_axis_corrected)], [0, 0], 'Color', 'k');
line([min(time_axis_corrected), max(time_axis_corrected)], [threshold_MI_ctvsnorm, threshold_MI_ctvsnorm], 'Color', 'k', 'LineStyle', '--');
line([min(time_axis_corrected), max(time_axis_corrected)], [-threshold_MI_ctvsnorm, -threshold_MI_ctvsnorm], 'Color', 'k', 'LineStyle', '--');

try
    [~, idx_max_mass_pos_MI_ctvsnorm] = max(masses_pos_MI_ctvsnorm);
    for idx = clusters_pos_MI_ctvsnorm.PixelIdxList{idx_max_mass_pos_MI_ctvsnorm}
        area(time_axis_corrected(idx), t_values_MI_ctvsnorm(idx), 'FaceColor', 'r', 'EdgeColor', 'none');
    end
catch
    disp('No positive clusters found');
end
...
try
    [~, idx_max_mass_neg_MI_ctvsnorm] = max(masses_neg_MI_ctvsnorm);
    for idx = clusters_neg_MI_ctvsnorm.PixelIdxList{idx_max_mass_neg_MI_ctvsnorm}
        area(time_axis_corrected(idx), t_values_MI_ctvsnorm(idx), 'FaceColor', 'b', 'EdgeColor', 'none');
    end
catch
    disp('No negative clusters found');
end
