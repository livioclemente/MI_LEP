...
for t1 = 1:Nt_cluster
    for t2 = (t1+1):Nt_cluster
        ...
        h0_dlpth(t1, t2) = II_tot2;
    end
end

% Calculate the difference between the groups
diff_h0 = h0_ct - h0_dlpth;
...
% Save file
save('ct_vs_dlpth_results.mat', 'diff_h0', 'diff_perm_t_values', 'clust_max', 'zmap');
...
[~, ~, ~, stats] = ttest2(erp_ct, erp_dlpth);
t_values_erp_ctvsdlpth = stats.tstat;

% Permuta i dati e calcola i valori t per ogni permutazione
perm_t_values_erp_ctvsdlpth = zeros(num_perms, Nt);
for p = 1:num_perms
    perm_data = [erp_ct; erp_dlpth];
    perm_data = perm_data(randperm(size(perm_data, 1)), :);
    perm_data_ct = perm_data(1:size(erp_ct, 1), :);
    perm_data_dlpth = perm_data(size(erp_ct, 1)+1:end, :);
    
    for sample = 1:Nt
        [~, ~, ~, stats] = ttest2(perm_data_ct(:, sample), perm_data_dlpth(:, sample));
        perm_t_values_erp_ctvsdlpth(p, sample) = stats.tstat;
    end
end

% Calcola i p-value
p_values_erp_ctvsdlpth = sum(perm_t_values_erp_ctvsdlpth > repmat(t_values_erp_ctvsdlpth, num_perms, 1)) / num_perms;

% Confronta i p-value con il livello alpha
significantly_different_erp_ctvsdlpth = p_values_erp_ctvsdlpth < alpha;

% Stampa i risultati
for sample = 1:Nt
    if significantly_different_erp_ctvsdlpth(sample)
        fprintf('Il campione %d è significativamente diverso tra le due condizioni (p = %.4f).\n', sample, p_values_erp_ctvsdlpth(sample));
    else
...
threshold_erp_ctvsdlpth = quantile(perm_t_values_erp_ctvsdlpth(:), 0.975);

% Identifica i cluster di campioni adiacenti per i quali il valore t supera la soglia
clusters_pos_erp_ctvsdlpth = bwconncomp(t_values_erp_ctvsdlpth > threshold_erp_ctvsdlpth);
clusters_neg_erp_ctvsdlpth = bwconncomp(t_values_erp_ctvsdlpth < -threshold_erp_ctvsdlpth);

% Calcola la statistica del cluster per ogni cluster come la somma dei valori t nel cluster
masses_pos_erp_ctvsdlpth = cellfun(@(I) sum(t_values_erp_ctvsdlpth(I)), clusters_pos_erp_ctvsdlpth.PixelIdxList);
masses_neg_erp_ctvsdlpth = cellfun(@(I) sum(abs(t_values_erp_ctvsdlpth(I))), clusters_neg_erp_ctvsdlpth.PixelIdxList);

% Trova il cluster positivo/negativo più grande
[max_mass_pos_erp_ctvsdlpth, ~] = max(masses_pos_erp_ctvsdlpth);
[max_mass_neg_erp_ctvsdlpth, ~] = max(masses_neg_erp_ctvsdlpth);

% Assicurati che time_axis_corrected_ct abbia la stessa lunghezza di t_values
assert(length(time_axis_corrected) == length(t_values_erp_ctvsdlpth), 'Time axis and t values must have the same length');

% Trova l'indice del cluster positivo/negativo più grande
[~, idx_max_mass_pos_erp_ctvsdlpth] = max(masses_pos_erp_ctvsdlpth);
[~, idx_max_mass_neg_erp_ctvsdlpth] = max(masses_neg_erp_ctvsdlpth);

% Save variables for plot
save('ERP_cluster_ct_vs_dlpth.mat', 'time_axis_corrected', 't_values_erp_ctvsdlpth', 'threshold_erp_ctvsdlpth', 'clusters_pos_erp_ctvsdlpth', 'clusters_neg_erp_ctvsdlpth', 'idx_max_mass_pos_erp_ctvsdlpth', 'idx_max_mass_neg_erp_ctvsdlpth');
...
% Calcola i valori t per ogni timepoint tra data_ct e data_pth
[~, ~, ~, stats] = ttest2(MI_ct, MI_dlpth);
t_values_MI_ctvsdlpth = stats.tstat;

% Permuta i dati e calcola i valori t per ogni permutazione
perm_t_values_MI_ctvsdlpth = zeros(num_perms, Nt);
for p = 1:num_perms
    perm_data = [MI_ct; MI_dlpth];
    perm_data = perm_data(randperm(size(perm_data, 1)), :);
    perm_data_ct = perm_data(1:size(MI_ct, 1), :);
    perm_data_dlpth = perm_data(size(MI_ct, 1)+1:end, :);
    
    for sample = 1:Nt
        [~, ~, ~, stats] = ttest2(perm_data_ct(:, sample), perm_data_dlpth(:, sample));
        perm_t_values_MI_ctvsdlpth(p, sample) = stats.tstat;
    end
end

% Calcola i p-value
p_values_MI_ctvsdlpth = sum(perm_t_values_MI_ctvsdlpth > repmat(t_values_MI_ctvsdlpth, num_perms, 1)) / num_perms;

% Confronta i p-value con il livello alpha
significantly_different_MI_ctvsdlpth = p_values_MI_ctvsdlpth < alpha;

% Stampa i risultati
for sample = 1:Nt
    if significantly_different_MI_ctvsdlpth(sample)
        fprintf('Il campione %d è significativamente diverso tra le due condizioni (p = %.4f).\n', sample, p_values_MI_ctvsdlpth(sample));
    else
        fprintf('Il campione %d non è significativamente diverso tra le due condizioni (p = %.4f).\n', sample, p_values_MI_ctvsdlpth(sample));
    end
end
...
% Soglia per l'identificazione dei cluster - qui sto usando la soglia che hai calcolato precedentemente
threshold_MI_ctvsdlpth = quantile(perm_t_values_MI_ctvsdlpth(:), 0.975);

% Identifica i cluster di campioni adiacenti per i quali il valore t supera la soglia
clusters_pos_MI_ctvsdlpth = bwconncomp(t_values_MI_ctvsdlpth > threshold_MI_ctvsdlpth);
clusters_neg_MI_ctvsdlpth = bwconncomp(t_values_MI_ctvsdlpth < -threshold_MI_ctvsdlpth);

% Calcola la statistica del cluster per ogni cluster come la somma dei valori t nel cluster
masses_pos_MI_ctvsdlpth = cellfun(@(I) sum(t_values_MI_ctvsdlpth(I)), clusters_pos_MI_ctvsdlpth.PixelIdxList);
masses_neg_MI_ctvsdlpth = cellfun(@(I) sum(abs(t_values_MI_ctvsdlpth(I))), clusters_neg_MI_ctvsdlpth.PixelIdxList);

% Trova il cluster positivo/negativo più grande
[max_mass_pos_MI_ctvsdlpth, ~] = max(masses_pos_MI_ctvsdlpth);
[max_mass_neg_MI_ctvsdlpth, ~] = max(masses_neg_MI_ctvsdlpth);

% Assicurati che time_axis_corrected_ct abbia la stessa lunghezza di t_values
assert(length(time_axis_corrected) == length(t_values_MI_ctvsdlpth), 'Time axis and t values must have the same length');

% Trova l'indice del cluster positivo/negativo più grande
[~, idx_max_mass_pos_MI_ctvsdlpth] = max(masses_pos_MI_ctvsdlpth);
[~, idx_max_mass_neg_MI_ctvsdlpth] = max(masses_neg_MI_ctvsdlpth);

% Save variables for plot
save('MI_cluster_ct_vs_dlpth.mat', 'time_axis_corrected', 't_values_MI_ctvsdlpth', 'threshold_MI_ctvsdlpth', 'clusters_pos_MI_ctvsdlpth', 'clusters_neg_MI_ctvsdlpth', 'idx_max_mass_pos_MI_ctvsdlpth', 'idx_max_mass_neg_MI_ctvsdlpth');
...
plot(time_plot, t_values_erp_ctvsdlpth, 'k');
hold on;
...
line([min(time_axis_corrected), max(time_axis_corrected)], [threshold_erp_ctvsdlpth, threshold_erp_ctvsdlpth], 'Color', 'k', 'LineStyle', '--');
line([min(time_axis_corrected), max(time_axis_corrected)], [-threshold_erp_ctvsdlpth, -threshold_erp_ctvsdlpth], 'Color', 'k', 'LineStyle', '--');

try
    [~, idx_max_mass_pos_erp_ctvsdlpth] = max(masses_pos_erp_ctvsdlpth);
    for idx = clusters_pos_erp_ctvsdlpth.PixelIdxList{idx_max_mass_pos_erp_ctvsdlpth}
        area(time_axis_corrected(idx), t_values_erp_ctvsdlpth(idx), 'FaceColor', 'r', 'EdgeColor', 'none');
    end
catch
...
try
    [~, idx_max_mass_neg_erp_ctvsdlpth] = max(masses_neg_erp_ctvsdlpth);
    for idx = clusters_neg_erp_ctvsdlpth.PixelIdxList{idx_max_mass_neg_erp_ctvsdlpth}
        area(time_axis_corrected(idx), t_values_erp_ctvsdlpth(idx), 'FaceColor', 'b', 'EdgeColor', 'none');
    end
catch
...
plot(time_axis_corrected, t_values_MI_ctvsdlpth, 'k');
hold on;
xlabel('Tempo');
ylabel('Valore t');

line([min(time_axis_corrected), max(time_axis_corrected)], [0, 0], 'Color', 'k');
line([min(time_axis_corrected), max(time_axis_corrected)], [threshold_MI_ctvsdlpth, threshold_MI_ctvsdlpth], 'Color', 'k', 'LineStyle', '--');
line([min(time_axis_corrected), max(time_axis_corrected)], [-threshold_MI_ctvsdlpth, -threshold_MI_ctvsdlpth], 'Color', 'k', 'LineStyle', '--');

try
    [~, idx_max_mass_pos_MI_ctvsdlpth] = max(masses_pos_MI_ctvsdlpth);
    for idx = clusters_pos_MI_ctvsdlpth.PixelIdxList{idx_max_mass_pos_MI_ctvsdlpth}
        area(time_axis_corrected(idx), t_values_MI_ctvsdlpth(idx), 'FaceColor', 'r', 'EdgeColor', 'none');
    end
catch
    disp('No positive clusters found');
end
...
try
    [~, idx_max_mass_neg_MI_ctvsdlpth] = max(masses_neg_MI_ctvsdlpth);
    for idx = clusters_neg_MI_ctvsdlpth.PixelIdxList{idx_max_mass_neg_MI_ctvsdlpth}
        area(time_axis_corrected(idx), t_values_MI_ctvsdlpth(idx), 'FaceColor', 'b', 'EdgeColor', 'none');
    end
catch
    disp('No negative clusters found');
end
